...
## Part 6 - Between sample diversity 
...
![image](https://github.com/user-attachments/assets/95c8e343-cf94-47ba-8370-075bf1877dbd)

- We have a PCoA plot now! 
**Question 6A Imagine two points on the PCoA plot above. In case one, these two points are distributed vertically and in case 2, the points are distributed horizontally (equidistant to case 1). In which case is there greater distance between points?**
...
**Question 6B) Which metadata has the largest effect on beta-diversity based on Bray-Curtis ordinations?**
...
**Question 6C) Qualitatively, which distance metric yields higher differences between the groups. Why would this be the case?**


- Let's try to answer a different question, the effect of diet on each vendor's microbiome. Like we did earlier, subset your phyloseq object for each vendor and generate Bray-Curtis PCoAs. In addition, we'll remove the baseline effects so that we can focus simply on the differences between groups post-intervention
...
**Question 6D) In which vendor are the diet effects most pronounced? Does resistant start or inulin generally have a larger effect compared to the control cellulose diet?**
...
- We'll need to perform this test on a distance matrix. Let's start by performing this on the whole dataset. We'll need to use Vegan's PERMANOVA test, called adonis
...
#Requires,the bray_distance matrix, the 
...
```
![image](https://github.com/user-attachments/assets/29e1948a-09d3-4ddd-8da9-865bdb53ad6e)

Here, we can see the output of the adonis test. The two important columns for us are the p column, which tells us that diet group does have a significant effect! The R2 column tells us what proportion of total variance of the data is explained by a certain factor (diet group) in this case. 


**Question 6E) ?**Which metadata groups have a significant effecr on beta-diversity? Which has the largest quantitative effect?**
...
```
**Question 6E) In which vendor is the effect of diet the largest? Does this match your observations with the corresponding PCoA plot?**
...
![image](https://github.com/user-attachments/assets/12b1ee29-34fb-489c-8904-f045e87daa15)
...
***Optional Question 6F) See what genera are driving diet differences for each vendor**
